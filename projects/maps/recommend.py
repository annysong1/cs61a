...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        new_cluster = group_by_centroid(restaurants, centroids)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    new_function = max(feature_fns, key = lambda a: find_predictor(user, reviewed, a) [1])

    return find_predictor(user, reviewed, new_function)[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            predictions[name] = int(user_rating(user, name))
        else:
