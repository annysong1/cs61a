...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            src.remove_front() #when you call src.remove_front() in this way, along with returning nil, wouldnt src.remove_front() remove and return the next token in src?#
            return nil
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2        
            src.remove_front() #This removes and returns the first token in the src
            next_src = scheme_read(src) #passing in the remaining src without the token in the front and sets it to the the name next
            if src.current() == ')': #passed the test but how come in the case of . 3), wouldnt src.current() be 3 not )??#
                src.remove_front()
                return next_src
            ...
            # BEGIN PROBLEM 1
            #when it says pair instance, why cant you do *return src.__init__(scheme_read(src), read_tail(src))* is it because that is for constructing?
