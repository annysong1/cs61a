...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        operator = scheme_eval(first, env) #binds operator to calling scheme_eval on the first expression in pair
        check_procedure(operator) #raises schemeerror if procedure is not callable
        operands = rest.map(lambda item: scheme_eval(item, env)) #now call map method (which is a method called on the rest of the pair) that applies a function that takes in 1 arg to every item in scheme list
        return scheme_apply(operator, operands, env) #now apply operator to operands
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        # BEGIN PROBLEM 11
        #when it says create a new frame instance, call frame(...) or like create a new lambdaprocedure instance, just call lambdaprocedure()
        #This is how you create a new instance is by Name_that_I_made_up = CLASSNAME(arguments to pass in) 
        child = Frame(self) #creating a new child with self as its parent
        if len(vals) != len(formals): #this checks if the length of argument values(val) is not equal to the length of formal parameters(formals)
            raise SchemeError
        while formals != nil:
            child.define(formals.first, vals.first) #Remember to not pass in all of formals and val but only the car of it so formals.first and val.first
            formals, vals = formals.second, vals.second #Remember to redefine formals and val to only pass in the cdr of it so formals.second and val.second
        return child
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while args is not nil:
            ...
            args = args.second
        # BEGIN PROBLEM 4
        #The if self.use_env... etc should go inside the try statement not, outside. 
        #remember that when returning self.fn(...) the inside should not be *args but self.fn(*python_args)
        #Why is it that self.fn(*python_args) is outside the if statement?#
        try:
            if self.use_env:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        open_new_frame = self.env.make_child_frame(self.formals, args)
        return open_new_frame
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(function_name, do_lambda_form(function_pair, env))
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    rest = expressions.second #binding the second part of the whole expression which is passed in as a Scheme list
    return LambdaProcedure(formals, rest, env) #returning the LambdaProcedure instance which passes in the first part, second part of expression, and environment
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    while expressions.second != nil:
        current_expression = scheme_eval(expressions.first, env)
        if scheme_truep(current_expression):
            pass
        else:
...
def do_or_form(expressions, env):
    ...
    while expressions.second != nil:
        current_expression = scheme_eval(expressions.first, env)
        if scheme_falsep(current_expression):
            pass
        else:
            return current_expression
        expressions = expressions.second
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            value = eval_all(clause.second, env) #value gets bound to the last evaluated subexpression in if <test> is true in (cond (<test> [(subexpr)(subexpr)]).....
            if value == None: #if there is no expression then...
                return test #the evaluator either returns True (if clause.first is "else") or the actual value of clause.first (if its not an else case)
            return value #in all other cases such as a regular case where the clause.first is True and there is onlhy one clause.second (meaning no subexpr) return value
            # END PROBLEM 14
        expressions = expressions.second
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            result = original_scheme_eval(expr, env, False)
        return result
        # END
