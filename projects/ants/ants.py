    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) and self.ant.is_container:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return self.is_container and (self.contained_ant == None) and (other.is_container == False)
    ...
    def action(self, colony):
        ...
        while place is not None:
            if place.ant:
                ...
                if place.ant.is_container and place.ant.contained_ant is not None and place.ant.contained_ant not in self.doubled_ants:
...
def make_slow(action, bee):
    ...
    def new_action(duration):
        def slow_action(colony):
            nonlocal duration
            if duration < 1 or colony.time % 2 == 0:
                action(colony)
            duration -= 1
        return slow_action
    return new_action
...
def make_scare(action, bee):
    ...
    def new_action(duration):
        def scared_action(colony):
            ...
            if duration < 1:
                bee.backwards = False
            elif not bee.scared and duration >= 1:
                bee.backwards = True
                bee.scared = True
            action(colony)
            duration -= 1
